...
class cleanRefs:
    ...
    def checkForDuplicates(self, checkItems, checkTag):
        if (sys.version_info[0] > 2 or (sys.version_info[0]==2  and sys.version_info[1]>6)) :
            chklist = [v[checkTag] for v in (vv[1] for i, vv in checkItems.items()) if checkTag in v and checkTag in self._refs]
            if (len(chklist) > len(set(chklist))):
                ...
                if (len(chklist) > len(set(chklist))):
                    print('No duplicate ',checkTag,'s',' in used references, but found duplicates in the bibfile',sep="")
                    print([v for v, vv in collections.Counter(chklist).items() if vv > 1])
        else:
            chklist = [v[checkTag] for v in (vv[1] for i, vv in checkItems.iteritems()) if checkTag in v and checkTag in self._refs]
            if (len(chklist) > len(set(chklist))):
                print('Have duplicate ',checkTag,'s',sep="")
